...
source] section of the documentation. I have already ran the scripts in the project so you can just `zig build` . In case you want to do it with a clean MuJoCo repo you should manually add the dependencies from the CMakeLists.txt and adapt the `build.zig` to the paths if necessary, after that run `zig build`. However, note that the commit at the tip of the `main` branch may be unstable.
